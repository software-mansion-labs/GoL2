...
func evaluate_rounds{
...
}(rounds: felt, cell_states: felt*) -> (cell_states: felt*) {
    alloc_locals;
    if (rounds == 0) {
    ...
    apply_rules(cell=DIM * DIM, cell_states=cell_states, pending_states=pending_states);
...
func apply_rules{
    syscall_ptr: felt*, bitwise_ptr: BitwiseBuiltin*, pedersen_ptr: HashBuiltin*, range_check_ptr
}(cell: felt, cell_states: felt*, pending_states: felt*) {
    alloc_locals;
    if (cell == 0) {
        return ();
    }

    apply_rules(cell=cell - 1, cell_states=cell_states, pending_states=pending_states);
...
func get_adjecent{range_check_ptr}(cell_idx: felt) -> (
    L: felt, R: felt, U: felt, D: felt, LU: felt, RU: felt, LD: felt, RD: felt
) {
    alloc_locals;
    ...
    return (L=L, R=R, U=U, D=D, LU=LU, RU=RU, LD=LD, RD=RD);
}
