...
from contracts.utils.constants import (
    CREATE_CREDIT_REQUIREMENT,
    INFINITE_GAME_GENESIS,
...
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    proxy_admin: felt, token_name: felt, token_symbol: felt, token_decimals: felt
) {
    alloc_locals;
...
func create{
    ...
    create_new_game(game_state=game_state, user_id=caller);
    return ();
}
...
func evolve{
    ...
    alloc_locals;
    let (caller) = ensure_user();
    let (generation, game) = evolve_game(game_id=game_id, user=caller);
    save_game(game_id=game_id, generation=generation, packed_game=game);
    save_generation_id(game_id=game_id, generation=generation);
...
func view_game{
    ...
    let (game_state) = get_game(game_id, generation);
    return (game_state,);
}

// Returns the current generation of a given game.
@view
func get_current_generation{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    game_id: felt
) -> (generation: felt) {
