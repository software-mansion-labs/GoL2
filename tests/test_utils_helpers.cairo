...
from contracts.utils.helpers import (
    pay,
    reward_user,
    ensure_user,
    evolve_game,
    ...
    save_generation_id,
    assert_game_exists,
    assert_game_does_not_exist,
    get_game,
    get_generation,
    assert_valid_new_game,
    create_new_game,
    get_last_state,
    assert_valid_cell_index,
    activate_cell,
)
...
@external
func test_pay{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    reward_user(USER_ID);
...
@external
func test_pay_not_enough_credits{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ) {
    reward_user(USER_ID);
...
@external
func test_reward_user{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    reward_user(USER_ID);
...
func test_get_game{
    ...
    create_new_game(game_state=INFINITE_GAME_GENESIS, user_id=USER_ID);
    let (game_state) = get_game(game_id=INFINITE_GAME_GENESIS, generation=1);
...
func test_get_game_does_not_exist{
    ...
    get_game(game_id=INFINITE_GAME_GENESIS, generation=50);
    return ();
}
...
func test_assert_invalid_new_game_already_exists{
    ...
    create_new_game(game_state=16, user_id=USER_ID);
...
@external
func test_get_last_state{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    create_new_game(game_state=INFINITE_GAME_GENESIS, user_id=USER_ID);
...
func test_activate_cell{
    ...
    activate_cell(generation=1, caller=USER_ID, cell_index=0, current_state=INFINITE_GAME_GENESIS);
...
func test_activate_cell_invalid_cell_index{
    ...
    activate_cell(
        generation=1, caller=USER_ID, cell_index=226, current_state=INFINITE_GAME_GENESIS
    );
    return ();
}
...
func test_activate_cell_nothing_changed{
    ...
    activate_cell(generation=1, caller=USER_ID, cell_index=99, current_state=INFINITE_GAME_GENESIS);
    return ();
}
